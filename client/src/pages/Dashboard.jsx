        ...
        <>
            ...
            <div className="container mx-auto px-4 py-8 relative z-10">
                <header className="mb-12 flex justify-between items-end">
                    <div>
                        <h1 className="text-6xl font-bold tracking-tighter mb-2 text-gradient">
                            ShunyAnatam
                        </h1>
                            ...
                            <Link to={`/project/${project._id}`} key={project._id} className="block group">
                                <div className="glass-card border-none bg-white/5 hover:bg-white/10 transition-all duration-300 transform group-hover:-translate-y-1 h-full flex flex-col items-start text-left">
                                    <div className="flex justify-between items-start w-full mb-4">
                                        ...
                                        ${project.status === 'COMPLETED' ? 'bg-green-400 shadow-[0_0_10px_rgba(74,222,128,0.5)]' :
                                                project.status === 'PROCESSING' ? 'bg-blue-400 animate-pulse' :
                                                    project.status === 'FAILED' ? 'bg-red-400' : 'bg-gray-400'}`}>
                                        </div>
                                        <span className="text-xs font-mono text-gray-500">{new Date(project.createdAt).toLocaleDateString()}</span>
                                    </div>
                                    ...
                                    <div className="w-full pt-4 border-t border-white/5 flex justify-between items-center">
                                        ...
                                        <span className="text-sm text-white/80 group-hover:translate-x-1 transition-transform">
                                            View →
                                        </span>
                                    </div>
                                </div>
