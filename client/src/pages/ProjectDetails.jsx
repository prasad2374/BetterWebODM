...
const ProjectDetails = () => {
    ...
    const filterContainedBoxes = (features) => {
        ...
        for (let i = 0; i < withBounds.length; i++) {
            for (let j = 0; j < withBounds.length; j++) {
                ...
                if (a.minLon >= b.minLon && a.maxLon <= b.maxLon &&
                    a.minLat >= b.minLat && a.maxLat <= b.maxLat) {

                    // A is inside B
                    // Handle Identical: Remove duplicate (keep first encountered/lower index)
                    if (Math.abs(a.minLon - b.minLon) < 1e-9 && Math.abs(a.maxLon - b.maxLon) < 1e-9 &&
                        Math.abs(a.minLat - b.minLat) < 1e-9 && Math.abs(a.maxLat - b.maxLat) < 1e-9) {
    ...
    const handleRunDetection = async () => {
        ...
        try {
            const baseUrl = import.meta.env.VITE_API_URL || 'http://localhost:5000/api';
            const res = await fetch(`${baseUrl}/projects/${id}/detect`, {
                method: 'POST',
                    ...
                    {project.status === 'COMPLETED' && (
                        <div className="flex bg-black/30 backdrop-blur-md rounded-full p-1 border border-white/10 space-x-1">
                            <button
                                ...
                                className={`px-5 py-2 rounded-full text-xs font-medium tracking-widest transition-all ${viewMode === '2D' ? 'bg-white text-black shadow-lg' : 'text-gray-400 hover:text-white'
                                    }`}
                            >
                                ORTHO
                            </button>
                            <button
                                ...
                                className={`px-5 py-2 rounded-full text-xs font-medium tracking-widest transition-all ${viewMode === '3D' ? 'bg-white text-black shadow-lg' : 'text-gray-400 hover:text-white'
                                    }`}
                            >
                                MODEL
                            </button>
                            ...
                            <div className="relative group/models">
                                ...
                                >
                                    <option value="" className="bg-black text-gray-400">Default Model</option>
                                    {models.map(m => (
                                        <option key={m} value={m} className="bg-black text-white">{m}</option>
                                    ))}
                                </select>
                                <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-xs text-gray-400">▼</div>
                            </div>

                            <button
                                onClick={handleRunDetection}
                                disabled={detecting}
                                className={`px-5 py-2 rounded-r-full text-xs font-medium tracking-widest transition-all -ml-1 border-l border-white/10 ${showingDetections ? 'bg-yellow-400 text-black shadow-lg' : 'bg-black/30 backdrop-blur-md text-gray-400 hover:text-white hover:bg-white/10 border border-white/10'
                                    }`}
                            >
            ...
            <div className="flex-1 relative bg-black">
                ...
                {project.status === 'COMPLETED' ? (
                    <div className="w-full h-full">
                        {viewMode === '2D' ? (
                            <MapViewer taskId={project._id} project={project} detections={filteredDetections} />
                        ) : (
