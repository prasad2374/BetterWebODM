			...
			<div className="bg-black/90 border border-white/10 rounded-2xl w-full max-w-[95vw] h-[95vh] overflow-hidden flex flex-col shadow-2xl">
				...
				<div className="flex-1 overflow-y-auto p-6">
					{activeTab === "height" && (
						<div className="space-y-8">
							<div className="p-4 rounded-xl bg-white/5 border border-white/10 text-gray-300 text-sm">
								<span className="text-white font-medium">Flight Height Calculator:</span> Determine the optimal flight
								altitude to achieve a specific Ground Sample Distance (GSD) for various camera models.
							</div>
							<form
								onSubmit={handleHeightCalc}
					...
					{activeTab === "focal" && (
						<div className="space-y-8">
							<div className="p-4 rounded-xl bg-white/5 border border-white/10 text-gray-300 text-sm">
								<span className="text-white font-medium">Metadata Analyzer:</span> Extract and analyze image EXIF data
								(Focal Length, Altitude) to calculate the effective Ground Sample Distance (GSD).
							</div>
									...
									<div className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-white/20 rounded-xl group-hover:border-white/40 group-hover:bg-white/5 transition-all">
										<div className="flex flex-col items-center justify-center pt-5 pb-6">
											<p className="mb-2 text-sm text-gray-400">
												<span className="font-semibold text-white">Click to upload</span> images for EXIF analysis
											</p>
					...
					{activeTab === "analysis" && (
						<div className="space-y-8">
							<div className="p-4 rounded-xl bg-white/5 border border-white/10 text-gray-300 text-sm">
								<span className="text-white font-medium">Drone Analysis:</span> Run object detection on sample images to
							...
							<form onSubmit={(e) => handleFileUpload(e, "analyze", setAnalysisLoading, setAnalysisResult)}>
								<label className="block w-full cursor-pointer group">
									<div className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-white/20 rounded-xl group-hover:border-white/40 group-hover:bg-white/5 transition-all">
										<div className="flex flex-col items-center justify-center pt-5 pb-6">
											<p className="mb-2 text-sm text-gray-400">
												<span className="font-semibold text-white">Click to upload</span> sample images
											</p>
								...
								<div className="space-y-6 animate-fade-in">
									<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
										...
										<div className="p-6 bg-white/5 border border-white/10 rounded-xl">
											...
											<div className="text-3xl font-bold text-blue-300">
												{analysisResult.best_config.avg_conf.toFixed(4)}
											</div>
										</div>
										<div className="p-6 bg-white/5 border border-white/10 rounded-xl">
											<div className="text-xs text-gray-500 uppercase mb-1">Calculated GSD</div>
											<div className="text-3xl font-bold text-green-300">
												{(analysisResult.best_config.gsd || 0).toFixed(2)} cm
											</div>
										</div>
									</div>
									...
									<div className="p-6 bg-gradient-to-r from-blue-900/20 to-purple-900/20 border border-white/10 rounded-xl">
										...
										<div className="grid grid-cols-2 gap-4 text-sm">
											...
											<div className="text-white font-mono text-right">{analysisResult.best_config.image}</div>
											<div className="text-gray-400">Altitude</div>
											<div className="text-white text-right">{analysisResult.best_config.alt_m} m</div>
											<div className="text-gray-400">Focal Length</div>
											<div className="text-white text-right">
												{analysisResult.best_config.focal_mm ? analysisResult.best_config.focal_mm.toFixed(2) : "N/A"}{" "}
												mm
											</div>
										</div>
									</div>
