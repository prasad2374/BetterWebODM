...
router.post("/est-focal", upload.array("images"), (req, res) => {
	...
	req.files.forEach((f) => {
		const newPath = join(batchDir, f.filename);
	...
	const py = spawn("python", [path.join(TOOLS_DIR, "est_focal.py"), "--dir", batchDir, "--model", modelArg]);
...
router.post("/analyze", upload.array("images"), (req, res) => {
	...
	const py = spawn("python", [path.join(TOOLS_DIR, "analyze.py"), "--dir", batchDir, "--model", modelArg]);
